...
test.describe('Ad-Builder Asset Upload API', () => {
  ...
  test.describe('Authentication', () => {
    ...
    test('AD_BUILDER-API-001 - returns 401 without token', async ({ request }) => {
      const res = await request.get('/api/ad-builder/frmassetupload');
  ...
  test.describe('Input Validation', () => {
    ...
    test('AD_BUILDER-API-011 - missing required fields returns 400', async ({ request }) => {
      const res = await request.post('/api/ad-builder/frmassetupload', {
        headers: { Authorization: `Bearer ${authToken}` },
        data: {},
      });
