...
## 2026-09-16 Follow-up: CI Failure - "2026 Flip to Samsung" Not Found on CurrentSPIFF

After the no-delete change above, a CI run intermittently (then consistently) failed with `Expected SPIFF program "2026 Flip to Samsung" to appear on CurrentSPIFF, but no matching row was found.` This is a direct, deterministic consequence of the no-delete decision, not flakiness: CurrentSPIFF's default listing is paginated at 10 rows and sorted newest-first, and every `SPIFF-MANAGE-001`/`SPIFF-PROCESS-001` run (locally and in CI) now leaves a new throwaway SPIFF behind. Confirmed live: by the time of this investigation, 8 of the first 10 rows were accumulated `QA Manage SPIFF Test .../QA Calc Verify SPIFF ...` records, pushing "2026 Flip to Samsung" onto page 2 ("Showing 1 to 10 of 12 entries"). This will keep getting worse with every future run, for every test that opens a claim form via `openClaimFormForProgram()` against that program name.

**Fix**: `SpiffClaimPage.navigateToCurrentSpiff()` now accepts an optional `programName` and, when given, uses CurrentSPIFF's own server-side "SPIFF Name" search filter (`?SpiffName=<value>` query param - confirmed live) instead of relying on the default unfiltered/paginated listing. `openClaimFormForProgram()` now always passes the program name through, so the lookup is robust regardless of how many other SPIFFs accumulate. Re-verified live: `SPIFF-SMOKE-003`, `SPIFF-TC-013`, `SPIFF-TC-014` all pass again.
